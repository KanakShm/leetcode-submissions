...
public:
    vector<int> getOrder(vector<vector<int>>& tasks) {
        ...
        */
        std::vector<int> res;
        int time = 0;
        std::priority_queue<std::pair<int, int>> enqueue;
        for (int i = 0; i < tasks.size(); ++i) {
            enqueue.push(std::make_pair(-tasks[i][0], -i));
        }

        std::priority_queue<std::pair<int, int>> process;
        while (!enqueue.empty() || !process.empty()) {
            ...
            while (!enqueue.empty() && enqueue.top().first >= time) {
                std::pair<int, int> top = enqueue.top();
                enqueue.pop();
                process.push(std::make_pair(-tasks[-top.second][1], top.second));
            }

            if (!process.empty()) {
                std::pair<int, int> top = process.top();
                process.pop();
